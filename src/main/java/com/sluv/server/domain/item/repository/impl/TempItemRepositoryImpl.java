...
public class TempItemRepositoryImpl implements TempItemRepositoryCustom {
    ...
    public Page<TempItem> getTempItemList(User user, Pageable pageable) {
        ...
        List<TempItem> content = jpaQueryFactory.selectFrom(tempItem)
                .leftJoin(tempItem.celeb, celeb).fetchJoin()
                ...
                .leftJoin(tempItem.category, itemCategory).fetchJoin()
                .leftJoin(itemCategory.parent, parentItemCategory).fetchJoin()
                .leftJoin(tempItem.brand, brand).fetchJoin()
                .leftJoin(tempItem.newCeleb, newCeleb).fetchJoin()
                .leftJoin(tempItem.newBrand, newBrand).fetchJoin()
    ...
    public List<TempItemResDto> getTempItemResDto(List<TempItem> content) {
        ...
        List<TempItemImg> tempItemImgs = jpaQueryFactory.selectFrom(tempItemImg)
                .where(tempItemImg.tempItem.id.in(tempItemIds))
                .fetch();
        ...
        Map<Long, List<ItemImgResDto>> tempItemImgMap = tempItemImgs.stream()
                .collect(Collectors.groupingBy(ti -> ti.getTempItem().getId(),
                        Collectors.mapping(ItemImgResDto::of, Collectors.toList())));

        Map<Long, List<ItemLinkResDto>> tempItemLinkMap = tempItemLinks.stream()
                .collect(Collectors.groupingBy(tl -> tl.getTempItem().getId(),
                        Collectors.mapping(ItemLinkResDto::of, Collectors.toList())));

        Map<Long, List<Hashtag>> tempItemHashtagMap = tempItemHashtags.stream()
                .collect(Collectors.groupingBy(th -> th.getTempItem().getId(),
                        Collectors.mapping(TempItemHashtag::getHashtag, Collectors.toList())));

        return content.stream()
                .map(ti -> TempItemResDto.of(ti,
                        tempItemImgMap.get(ti.getId()),
                        tempItemLinkMap.get(ti.getId()),
                        tempItemHashtagMap.get(ti.getId()))
                ).toList();
    }
}
